...
abstract class Categories implements ActiveRecordInterface
{
     ...
     * TableMap class name
     */
    const TABLE_MAP = '\\Map\\CategoriesTableMap';
    ...
    /**
     * @var        ObjectCollection|ChildSubcategories[] Collection to store aggregation of ChildSubcategories objects.
     */
    protected $collSubcategoriess;
    protected $collSubcategoriessPartial;
    ...
    /**
     * An array of objects scheduled for deletion.
     * @var ObjectCollection|ChildSubcategories[]
     */
    protected $subcategoriessScheduledForDeletion = null;

    /**
     * Initializes internal state of Base\Categories object.
     */
     ...
     * @return $this|\Categories The current object (for fluent API support)
     */
    public function setCategoryId($v)
    {
        ...
        if ($this->category_id !== $v) {
            $this->category_id = $v;
            $this->modifiedColumns[CategoriesTableMap::COL_CATEGORY_ID] = true;
        }
    ...
    public function hydrate($row, $startcol = 0, $rehydrate = false, $indexType = TableMap::TYPE_NUM)
    {
        try {
            ...
            $col = $row[TableMap::TYPE_NUM == $indexType ? 0 + $startcol : CategoriesTableMap::translateFieldName('CategoryId', TableMap::TYPE_PHPNAME, $indexType)];
            ...
            return $startcol + 2; // 2 = CategoriesTableMap::NUM_HYDRATE_COLUMNS.

        } catch (Exception $e) {
            throw new PropelException(sprintf('Error populating %s object', '\\Categories'), 0, $e);
        }
    }
        ...
        if (!$this->alreadyInSave) {
            ...
            if ($this->itemssScheduledForDeletion !== null) {
                if (!$this->itemssScheduledForDeletion->isEmpty()) {
                    \ItemsQuery::create()
                        ->filterByPrimaryKeys($this->itemssScheduledForDeletion->getPrimaryKeys(false))
                        ->delete($con);
            ...
            if ($this->subcategoriessScheduledForDeletion !== null) {
                if (!$this->subcategoriessScheduledForDeletion->isEmpty()) {
                    \SubcategoriesQuery::create()
                        ->filterByPrimaryKeys($this->subcategoriessScheduledForDeletion->getPrimaryKeys(false))
                        ->delete($con);
                    $this->subcategoriessScheduledForDeletion = null;
                }
            }
    ...
    protected function doInsert(ConnectionInterface $con)
    {
        ...
        if (null !== $this->category_id) {
            throw new PropelException('Cannot insert a value for auto-increment primary key (' . CategoriesTableMap::COL_CATEGORY_ID . ')');
        }

         // check the columns in natural order for more readable SQL queries
        if ($this->isColumnModified(CategoriesTableMap::COL_CATEGORY_ID)) {
    ...
    public function toArray($keyType = TableMap::TYPE_PHPNAME, $includeLazyLoadColumns = true, $alreadyDumpedObjects = array(), $includeForeignObjects = false)
    {
        ...
        if ($includeForeignObjects) {
            ...
            if (null !== $this->collSubcategoriess) {
                ...
                $result[$key] = $this->collSubcategoriess->toArray(null, false, $keyType, $includeLazyLoadColumns, $alreadyDumpedObjects);
            }
        }
    ...
    public function buildCriteria()
    {
        ...
        if ($this->isColumnModified(CategoriesTableMap::COL_CATEGORY_ID)) {
            $criteria->add(CategoriesTableMap::COL_CATEGORY_ID, $this->category_id);
        }
        if ($this->isColumnModified(CategoriesTableMap::COL_CATEGORY_NAME)) {
    ...
    public function initRelation($relationName)
    {
        ...
        if ('Subcategories' == $relationName) {
            return $this->initSubcategoriess();
        }
    }
    ...
    public function setItemss(Collection $itemss, ConnectionInterface $con = null)
    {
        ...
        foreach ($itemssToDelete as $itemsRemoved) {
            $itemsRemoved->setCategories(null);
        }
    ...
    public function removeItems(ChildItems $items)
    {
        if ($this->getItemss()->contains($items)) {
            ...
            $this->itemssScheduledForDeletion[]= clone $items;
            $items->setCategories(null);
        }
     ...
     * @return ObjectCollection|ChildItems[] List of ChildItems objects
     */
    public function getItemssJoinSubcategories(Criteria $criteria = null, ConnectionInterface $con = null, $joinBehavior = Criteria::LEFT_JOIN)
    {
        $query = ChildItemsQuery::create(null, $criteria);
        $query->joinWith('Subcategories', $joinBehavior);
    ...
    /**
     * Clears out the collSubcategoriess collection
     *
     * This does not modify the database; however, it will remove any associated objects, causing
     * them to be refetched by subsequent calls to accessor method.
     *
    ...
    /**
     * Reset is the collSubcategoriess collection loaded partially.
     */
    public function resetPartialSubcategoriess($v = true)
    {
        $this->collSubcategoriessPartial = $v;
    }

    /**
     * Initializes the collSubcategoriess collection.
     *
     ...
     * @return void
     */
    public function initSubcategoriess($overrideExisting = true)
    {
        if (null !== $this->collSubcategoriess && !$overrideExisting) {
            return;
        }
        $this->collSubcategoriess = new ObjectCollection();
        $this->collSubcategoriess->setModel('\Subcategories');
    }

    /**
     * Gets an array of ChildSubcategories objects which contain a foreign key that references this object.
     *
     * If the $criteria is not null, it is used to always fetch the results from the database.
     * Otherwise the results are fetched from the database the first time, then cached.
     * Next time the same method is called without $criteria, the cached collection is returned.
     ...
     * @param      Criteria $criteria optional Criteria object to narrow the query
     * @param      ConnectionInterface $con optional connection object
     * @return ObjectCollection|ChildSubcategories[] List of ChildSubcategories objects
     * @throws PropelException
     */
    public function getSubcategoriess(Criteria $criteria = null, ConnectionInterface $con = null)
    {
        $partial = $this->collSubcategoriessPartial && !$this->isNew();
        if (null === $this->collSubcategoriess || null !== $criteria  || $partial) {
            if ($this->isNew() && null === $this->collSubcategoriess) {
                // return empty collection
                $this->initSubcategoriess();
            } else {
                $collSubcategoriess = ChildSubcategoriesQuery::create(null, $criteria)
                    ->filterByCategories($this)
                    ->find($con);

                if (null !== $criteria) {
                    if (false !== $this->collSubcategoriessPartial && count($collSubcategoriess)) {
                        $this->initSubcategoriess(false);

                        foreach ($collSubcategoriess as $obj) {
                            if (false == $this->collSubcategoriess->contains($obj)) {
                                $this->collSubcategoriess->append($obj);
                            }
                        }

                        $this->collSubcategoriessPartial = true;
                    }

                    return $collSubcategoriess;
                }

                if ($partial && $this->collSubcategoriess) {
                    foreach ($this->collSubcategoriess as $obj) {
                        if ($obj->isNew()) {
                            $collSubcategoriess[] = $obj;
                        }
                    }
                }

                $this->collSubcategoriess = $collSubcategoriess;
                $this->collSubcategoriessPartial = false;
            }
        }
    ...
    /**
     * Sets a collection of ChildSubcategories objects related by a one-to-many relationship
     * to the current object.
     * It will also schedule objects for deletion based on a diff between old objects (aka persisted)
     * and new objects from the given Propel collection.
     *
     * @param      Collection $subcategoriess A Propel collection.
     * @param      ConnectionInterface $con Optional connection object
     * @return $this|ChildCategories The current object (for fluent API support)
     */
    public function setSubcategoriess(Collection $subcategoriess, ConnectionInterface $con = null)
    {
        /** @var ChildSubcategories[] $subcategoriessToDelete */
        $subcategoriessToDelete = $this->getSubcategoriess(new Criteria(), $con)->diff($subcategoriess);


        $this->subcategoriessScheduledForDeletion = $subcategoriessToDelete;

        foreach ($subcategoriessToDelete as $subcategoriesRemoved) {
            $subcategoriesRemoved->setCategories(null);
        }
        ...
        $this->collSubcategoriess = $subcategoriess;
        $this->collSubcategoriessPartial = false;
     ...
     * @throws PropelException
     */
    public function countSubcategoriess(Criteria $criteria = null, $distinct = false, ConnectionInterface $con = null)
    {
        $partial = $this->collSubcategoriessPartial && !$this->isNew();
        if (null === $this->collSubcategoriess || null !== $criteria || $partial) {
            if ($this->isNew() && null === $this->collSubcategoriess) {
            ...
            if ($partial && !$criteria) {
                return count($this->getSubcategoriess());
            }

            $query = ChildSubcategoriesQuery::create(null, $criteria);
            if ($distinct) {
                $query->distinct();
            }
        ...
        return count($this->collSubcategoriess);
    }

    /**
     * Method called to associate a ChildSubcategories object to this object
     * through the ChildSubcategories foreign key attribute.
     *
     * @param  ChildSubcategories $l ChildSubcategories
     * @return $this|\Categories The current object (for fluent API support)
     */
    public function addSubcategories(ChildSubcategories $l)
    {
        if ($this->collSubcategoriess === null) {
            $this->initSubcategoriess();
            $this->collSubcategoriessPartial = true;
        ...
        if (!$this->collSubcategoriess->contains($l)) {
            $this->doAddSubcategories($l);
        }
    ...
    /**
     * @param  ChildSubcategories $subcategories The ChildSubcategories object to remove.
     * @return $this|ChildCategories The current object (for fluent API support)
     */
    public function removeSubcategories(ChildSubcategories $subcategories)
    {
        if ($this->getSubcategoriess()->contains($subcategories)) {
            $pos = $this->collSubcategoriess->search($subcategories);
            $this->collSubcategoriess->remove($pos);
            if (null === $this->subcategoriessScheduledForDeletion) {
                $this->subcategoriessScheduledForDeletion = clone $this->collSubcategoriess;
                $this->subcategoriessScheduledForDeletion->clear();
            }
            $this->subcategoriessScheduledForDeletion[]= clone $subcategories;
    ...
    public function clearAllReferences($deep = false)
    {
        if ($deep) {
            ...
            if ($this->collSubcategoriess) {
                foreach ($this->collSubcategoriess as $o) {
                    $o->clearAllReferences($deep);
                }
            }
        } // if ($deep)
